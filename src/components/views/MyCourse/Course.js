...
function Course({ course, type = "Online" }) {
    return (
        <div className="col-lg-4 col-md-6 col-12 mb-4">
            <div className="rbt-card variation-01 rbt-hover">
                <div className="rbt-card-img">
                    <Link to={type === "Online" ? `/course-online/${course.slug}` : `/subject-offline/${course.slug}`}>
                ...
                <div className="rbt-card-body">
                    <div className="rbt-card-top mb-3">
                        <span className="rbt-badge variation-02 bg-secondary-opacity m-0">{formatLevelCourse(course.level)}</span>
                    ...
                    <h4 className="rbt-card-title">
                        <Link to={type === "Online" ? `/course-online/${course.slug}` : `/subject-offline/${course.slug}`} className="font-system">
                            {course.name}
                        </Link>
                    </h4>
                    ...
                    <ul className="rbt-meta">
                        <li>
                            <i className="feather-book"></i> {course.language}
                        </li>
                        <li>
                    ...
                    {course.description && <p className="rbt-card-text line-clamp-1">{course.description}</p>}

                    <div className="rbt-card-bottom">
                        <div className="rbt-price">
                            <span className="current-price">${course.price && course.price.toFixed(2)}</span>
                        </div>
                        <Link to={type === "Online" ? `/course-online/${course.slug}` : `/subject-offline/${course.slug}`} className="rbt-btn-link">
                            Go to course<i className="feather-arrow-right"></i>
                        </Link>
                    </div>
                </div>
                        ...
                        <div className="single-progress">
                            ...
                            <div className="progress">
                                <div
                                    ...
                                    role="progressbar"
                                    style={{
                                        width: `${(course.progress * 365) / 100}px`,
                                        visibility: "visible",
